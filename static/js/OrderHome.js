...
UpdateOrderPrice = function() {
    ...
    $("#orderPrice").html("NT$ " + orderPrice.toFixed(2));
};
    ...
    $("#OrderSubmit")
        ...
        .click(function() {
            if (window.order.foodList.length == 0) {
                bs4pop.notice("請選擇菜品！", { type: "danger" });
                return;
            } else if (window.table == 0) {
                bs4pop.notice("請選擇桌號！", { type: "danger" });
                return;
            }
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (left_amount < window.order.foodList[i].amount) {
                    bs4pop.notice("數量不足！", { type: "danger" });
                    return;
                }
            }

            bs4pop.confirm(
                "請確認桌號: {0} !".format(window.table),
                function(sure) {
                    if (!sure) {
                        return;
                    }
